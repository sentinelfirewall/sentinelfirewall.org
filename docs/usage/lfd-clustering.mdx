...
The `CLUSTER_KEY` is a secret key used to encrypt cluster communications using [the Blowfish algorithm](https://en.wikipedia.org/wiki/Blowfish_(cipher)). It should be between `8` and `56` ASCII characters long, longer is better, and must be the same on all members of the cluster.
