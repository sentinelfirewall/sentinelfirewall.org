...
8.  There are restrictions imposed by CloudFlare to the number of rules that can be created depending on the type of account used. See [Cloudflare IP Access rules](https://sentinelfirewall.org/redirect?to=https://developers.cloudflare.com/waf/tools/ip-access-rules/] for more information
...
`CF_TEMP` should be configured taking into account the maximum number of rules that the CloudFlare account allows: [https://developers.cloudflare.com/waf/tools/ip-access-rules/](https://sentinelfirewall.org/redirect?to=https://developers.cloudflare.com/waf/tools/ip-access-rules/)
...
> **NOTE**: You should always list the CloudFlare IP addresses in `/etc/csf/csf.ignore` to prevent them from being blocked by lfd from [https://www.cloudflare.com/ips/](https://sentinelfirewall.org/redirect?to=https://www.cloudflare.com/ips/)
